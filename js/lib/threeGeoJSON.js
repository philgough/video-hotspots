...
function convertToSphereCoords(coordinates_array, sphere_radius) {
    ...
    x_values.push(Math.cos(lat * Math.PI/180) * Math.cos(lon * Math.PI/180) * sphere_radius * -1); // multiply by -1, since we are on the inside of the sphere
...
function get_shape_geometry(values_axis1, values_axis2, values_axis3) {
    ...
    for (var i = 0; i < values_axis1.length; i++) {
        o_g.push(new THREE.Vector3(values_axis1[i],
                values_axis2[i], values_axis3[i]));
    }
    

    var shape = new THREE.Shape(o_g);
